...
<div style="margin:1% 0 0 1%; width:100%">
  <div style="margin:10px 0">
    ...
    <el-table-column label="等级类别编号" prop="id" sortable />
    <el-table-column label="每种等级最低分数" prop="lowerlimit" sortable />
    <el-table-column label="每种等级最高分数" prop="upperlimit" sortable/>
    <el-table-column label="等级名称" prop="leveltxt" sortable/>
    <el-table-column label="等级标识图片" prop="levelimg" sortable/>
    <el-table-column label="等级图片预览">
      <template #default="scope">
      <el-image
      style="width: 100px; height: 100px"
      :src="this.url+scope.row.levelimg"
  ...
  <div style="margin:10px 0">
    ...
    :before-close="handleClose"
     >
              ...
              <el-form-item label="每种等级最低分数">
                <el-input v-model="form.lowerlimit" />
              </el-form-item>
              <el-form-item label="每种等级最高分数">
                <el-input v-model="form.upperlimit" type="textarea"/>
              </el-form-item>
              <el-form-item label="等级名称">
                <el-input v-model="form.leveltxt" type="textarea"/>
              </el-form-item>
              <el-form-item label="等级标识图片">
                <el-input v-model="form.levelimg" type="textarea"/>
              </el-form-item>
            </el-form>
    <template #footer>
...
export default {
 ...
 methods: {
  ...
  //  保存
   save(){
     if(this.form.id){//更新
        request.put("/contribution",this.form).then(res=>{
      ...
      if(this.form.levelimg&&this.form.leveltxt&&this.form.lowerlimit&&this.form.upperlimit){
     request.post("/contribution",this.form).then(res=>{
       //console.log(res);
       if(res.code==='200'){
       this.$message({
         type:"success",
  ...
  confirmEvent(index,row) {
    ...
    request.delete("/contribution/"+row.id).then(res=>{
      if(res.code==='200'){
      this.$message({
        type:"success",
         message:"删除成功"
